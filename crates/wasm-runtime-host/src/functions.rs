...
impl SqliteStore {
  ...
  pub async fn initialize_sqlite_functions(
    &self,
    args: crate::InitArgs,
  ) -> Result<Vec<SqliteFunction>, Error> {
    ...
    let manifest = store
      .run_concurrent(
        async |accessor| -> Result<trailbase_wasm_common::manifest::InitManifest, Error> {
    ...
    return Ok(manifest.sqlite_functions.unwrap_or_default());
  }
...
pub fn setup_connection(
  ...
  for function in functions {
    match function {
      SqliteFunction::Scalar(function) => {
        ...
        let flags = {
          if function.flags.is_empty() {
            ...
            for flag in &function.flags {
              flags |= from_json_sqlite_function_flags(flag);
            }
            flags
          }
        };
...
fn from_json_sqlite_function_flags(
  f: &trailbase_wasm_common::manifest::SqliteFunctionFlag,
) -> rusqlite::functions::FunctionFlags {
  use rusqlite::functions::FunctionFlags as F;
  use trailbase_wasm_common::manifest::SqliteFunctionFlag;

  return match f {
    SqliteFunctionFlag::Utf8 => F::SQLITE_UTF8,
    SqliteFunctionFlag::Utf16le => F::SQLITE_UTF16LE,
    SqliteFunctionFlag::Utf16be => F::SQLITE_UTF16BE,
    SqliteFunctionFlag::Utf16 => F::SQLITE_UTF16,
    SqliteFunctionFlag::Deterministic => F::SQLITE_DETERMINISTIC,
    SqliteFunctionFlag::DirectOnly => F::SQLITE_DIRECTONLY,
    SqliteFunctionFlag::Subtype => F::SQLITE_SUBTYPE,
    SqliteFunctionFlag::Innocuous => F::SQLITE_INNOCUOUS,
    SqliteFunctionFlag::ResultSubtype => F::SQLITE_RESULT_SUBTYPE,
    SqliteFunctionFlag::Selforder1 => F::SQLITE_SELFORDER1,
  };
}
