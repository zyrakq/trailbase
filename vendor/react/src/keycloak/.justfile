old_repo := "registry.test:80"

build:
  jar -cvf providers/argiago.jar theme META-INF;

werf-dev-convert:
  kompose convert -f docker-compose.dev.yml -o ./.helm/templates;
  rm ./.helm/templates/*-persistentvolumeclaim.yaml;
  rm ./.helm/templates/*-networkpolicy.yaml;
  find ./.helm/templates -type f -exec sed -i "s/'{{{{ \(.*\) }}'/{{{{ \1 }}/g" {} +;
  find ./.helm/templates -type f -exec sed -i "s/\.values/\.Values/g" {} +;

werf-dev-up *FLAGS:
  werf converge --config='werf.dev.yaml' --repo {{old_repo}}/argiago {{FLAGS}}
werf-dev-down *FLAGS:
  werf dismiss --config='werf.dev.yaml' --repo {{old_repo}}/argiago {{FLAGS}}
werf-dev-cleanup *FLAGS:
  werf cleanup --config='werf.dev.yaml' --repo {{old_repo}}/argiago {{FLAGS}}