...
/**
 * `<trail-profile-account>` — destructive account deletion with confirmation modal.
 *
 ...
 * trail-profile-account-deleted for the host to handle redirect.
 *
...
export class TrailProfileAccount extends LitElement {
  ...
  private handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && this.state !== 'idle' && this.state !== 'deleting') {
      this.closeModal();
    }
  };
  ...
  private async handleConfirmDelete() {
    ...
    try {
      ...
      this.dispatchEvent(
        new CustomEvent('trail-profile-account-deleted', {
          bubbles: true,
          composed: true,
        })
      );
...
declare global {
  interface HTMLElementTagNameMap {
    'trail-profile-account': TrailProfileAccount;
  }
}
