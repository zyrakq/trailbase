...
import './trail-profile-avatar.ts';
  ...
  async connectedCallback() {
    ...
    try {
      ...
      if (u) {
        ...
        this.user = { id: '', email: this.email, hasMfa: this.hasMfa, csrfToken: '' };
      }
    } catch {
      if (this.email) {
        this.user = { id: '', email: this.email, hasMfa: this.hasMfa, csrfToken: '' };
    ...
    return html`
      <div class="profile-root">
        ...
        ${this.showChangePassword
          ...
          : ''}
        ${this.showOtpSection
          ? html`<trail-profile-totp .hasMfa=${this.user?.hasMfa ?? false}></trail-profile-totp>`
          : ''}
        <div class="actions">
          <button
