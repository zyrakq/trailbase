...
describe('app-header', () => {
  ...
  it('does not render the hamburger button when unauthenticated', async () => {
    element = document.createElement('app-header') as AppHeader;
    document.body.appendChild(element);
    await element.updateComplete;

    expect(element.shadowRoot?.querySelector('button.menu-btn')).toBeNull();
  });

  it('renders the hamburger button when authenticated', async () => {
    vi.mocked(authService.isAuthenticated).mockReturnValue(true);
  ...
  it('dispatches a menu-toggle CustomEvent (bubbles, composed) on hamburger click', async () => {
    vi.mocked(authService.isAuthenticated).mockReturnValue(true);
    ...
    expect(events[0].type).toBe('menu-toggle');
    expect(events[0].bubbles).toBe(true);
    expect(events[0].composed).toBe(true);

    element.removeEventListener('menu-toggle', handler);
  });
});
